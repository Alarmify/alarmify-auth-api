...
func RegisterAuthRoutes(r *Router, handler *handlers.AuthHandler) {
	v1 := r.Group("/api/v1")
	{
		auth := v1.Group("/auth")
		{
			...
			auth.POST("/mfa/verify", handler.VerifyMFA)
			auth.POST("/mfa/enable", handler.EnableMFA)
			auth.GET("/sso/providers", handler.GetSSOProviders)
		}
	}
}
